...
def find_measures_arrays(
    net_orig, trainloader, dataload_info, device, 
    measure_names=None, attacker=None, loss_fn=F.cross_entropy, 
    ...
    if attacker is not None:
        inputs = attacker.attack(inputs, targets)
        ...
        try:    
            for measure_name in measure_names:
                if measure_name not in measure_values:
                    val = measures.calc_measure(measure_name, net_orig, device, inputs, targets, loss_fn=loss_fn, split_data=ds, search_space=search_space)
...
def find_measures(
    ...
    if measures_arr is None:
        measures_arr = find_measures_arrays(
            net_orig, dataloader, dataload_info, device, 
            attacker=attacker, loss_fn=loss_fn, measure_names=measure_names, 
